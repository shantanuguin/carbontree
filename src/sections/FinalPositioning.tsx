export default function FinalPositioning() {
  return (
    <section className="bg-white py-24 lg:py-32 print-section-padding print-page-break">
      <div className="section-container">
        <div className="max-w-[800px] mx-auto text-center reveal">
          {/* Decorative line */}
          <div className="w-20 h-0.5 bg-[#8a9b5b] mx-auto mb-10" />

          {/* Quote */}
          <blockquote className="text-xl md:text-[28px] font-normal text-black leading-relaxed italic">
            "Carbon Tree is the fabric-led sustainable essentials brand for the conscious
            Indian consumer — offering premium material innovation at accessible prices, one
            thoughtful layer at a time."
          </blockquote>

          {/* Attribution */}
          <p className="mt-8 text-sm font-medium text-[#6b6b6b]">
            — Final Positioning Statement
          </p>
        </div>
      </div>
    </section>
  );
}
